...
- [x] Quick-add task inline on the board — "+ Add task" affordance per column, Enter to add and stays open for rapid successive adds; optimistically inserted via a lightweight stub (`use-create-task.ts`) then replaced by the real row on refetch since `task.create`'s response has no joined relations; respects the column's WIP limit the same way drag-and-drop does
